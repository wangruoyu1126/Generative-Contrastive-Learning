...
def get_dataset(dset, root, split, transform):
    try:
        return dset(root, train=(split == 'train'), transform=transform, download=False)
    except:
        return dset(root, split=split, transform=transform, download=False)
    ...
    def __init__(self, dset, root, num_classes, batch_size, transform):
        ...
        for cl in self.cl_list:
            sub_dataset = SubDataset(self.sub_meta[cl], cl,
                                     transform=transform if dset in [datasets.CIFAR10, datasets.CIFAR100] else identity)
