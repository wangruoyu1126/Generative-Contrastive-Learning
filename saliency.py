...
def get_saliency_mbd(input_img, method='b'):
    # img = input_img.detach().numpy()
    img = input_img
    ...
    if method == 'b':
        ...
        # Ruoyu - corner case handling
        if np.linalg.det(cov_left) == 0:
            cov_left = np.random.rand(cov_left.shape[0], cov_left.shape[1])
        if np.linalg.det(cov_right) == 0:
            cov_right = np.random.rand(cov_right.shape[0], cov_right.shape[1])
        if np.linalg.det(cov_top) == 0:
            cov_top = np.random.rand(cov_top.shape[0], cov_top.shape[1])
        if np.linalg.det(cov_bottom) == 0:
            cov_bottom = np.random.rand(cov_bottom.shape[0], cov_bottom.shape[1])
...
def get_bin_saliency_batch(input_batch):
    out = []
    i = 0
    for img in input_batch:
        # print('working on', i)
        sal_map = get_saliency_mbd(img)
        sal_map_bin = binarise_saliency_map(sal_map)
        out.append(sal_map_bin)
        # cv2.imwrite('org_{}.jpg'.format(i), img.detach().numpy().astype('uint8'))
        # cv2.imwrite('mask_{}.jpg'.format(i), 255 * sal_map_bin.astype('uint8'))
        i += 1
    # input_batch = input_batch.cpu().detach().numpy()
    # sal_map = np.apply_along_axis(get_saliency_mbd, 0, input_batch)
    # print('sal_map shape', sal_map.shape)
    # sal_map_bin = np.apply_along_axis(binarise_saliency_map, 0, sal_map)
    # print('sal_map_bin shape', sal_map_bin.shape)
    return out
