...
from model import Model, PretrainedResNet50, PretrainedResNet18, PretrainedResNet34
import random
...
def train(net, data_loader, train_optimizer, temperature, debiased, tau_plus, hard_neg, beta):
    ...
    for pos_1, pos_2, _, _, _ in train_bar:
        ...
        if hard_neg:
            ...
            # constrain (optional)
            Ng = torch.clamp(Ng, min=N * np.e ** (-1 / temperature))
        if debiased == False and hard_neg == False:
            Ng = neg.sum(dim=-1)
...
def test(net, memory_data_loader, test_data_loader):
    ...
    with torch.no_grad():
        ...
        # for data, _, _, _, target in tqdm(memory_data_loader, desc='Feature extracting'):
        for data, _, _, _, target in tqdm(memory_data_loader, desc='Feature extracting'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hard_neg', default=False, type=bool, help='Use hard negative sample(HCL) Loss?')
    parser.add_argument('--beta', default=1.0, type=float, help='beta in HCL')
    ...
    hard_neg, beta = args.hard_neg, args.beta
    ...
    if args.dataset == 'CIFAR10':
        ...
        print('train on NICO')
        train_data = utils.NICODataset(transform=utils.train_transform, train=True)
        train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True, num_workers=16, pin_memory=True,
                                  drop_last=True)
        memory_data = utils.NICODataset(transform=utils.test_transform, train=True)
        memory_loader = DataLoader(memory_data, batch_size=batch_size, shuffle=False, num_workers=16, pin_memory=True)
        test_data = utils.NICODataset(transform=utils.test_transform, train=False)
        test_loader = DataLoader(test_data, batch_size=batch_size, shuffle=False, num_workers=16, pin_memory=True)
    ...
    if args.pretrained_backbone:
        ...
        model = PretrainedResNet18(feature_dim).cuda()
    ...
    # img_shape = (3,32,32) # hard code for now
    # print('image shape', img_shape)

    # Generator setup
    # generator = GeneratorResNet(input_shape=(3, 32, 32), num_residual_blocks=0).cuda()
    # generator = nn.DataParallel(generator, )
    # optimizer_gen = optim.Adam(generator.parameters(), lr=args.lr_gen, weight_decay=1e-6)

    # generator = GeneratorResNet(input_shape=(3,32,32), num_residual_blocks=0).cuda()
    # generator.load_state_dict(torch.load('results/pretrain_generator.pth'))
    # generator = nn.DataParallel(generator, )
    # # optimizer_gen = optim.Adam(generator.parameters(), lr=args.lr_gen, weight_decay=0)
    # optimizer_gen = optim.SGD(generator.parameters(), lr=args.lr_gen)
    # print('done loading pretrained generator!')
    ...
    for epoch in range(1, epochs + 1):
        if args.baseline:
            ...
            # train_loss_gen = train_generator(model, generator,
            #                                  train_loader, optimizer_gen,
            #                                  temperature, pos_gen_weight)
