...
#         if train:
#             self.data_root = 'data/CIFAR10/train'
#         else:
#             self.data_root = 'data/CIFAR10/test'
#         self.img_root = self.data_root + '/img'
#         self.mask_root = self.data_root + '/mask'
#         self.pos_root = self.data_root + '/pos'
#         self.neg_root = self.data_root + '/neg'
#         self.imgs = []
#         self.masks = []
#         self.poss = []
#         self.negs = []
...
#             img = cv2.imread(self.img_root+'/' + filename)
#             mask = cv2.imread(self.mask_root + '/' + filename)
#             pos = cv2.imread(self.pos_root + '/' + filename)
#             neg = cv2.imread(self.neg_root + '/' + filename)
...
#         neg_gen = test_transform(neg_gen)
#
#         return pos_1, pos_2, pos_3, img, mask, pos_gen, neg_gen
...
class VOCDecFolder(Dataset):
    """
    Custom Dataset to load original img, positive generation and negative generation
    """
    def __init__(self, transform, train):
        if train:
            data_root = 'data/VOC_Dec/train'
            data_len = 5717
        else:
            data_root = 'data/VOC_Dec/test'
            data_len = 5823
        img_root = data_root + '/img'
        pos_root = data_root + '/pos'
        target_root = data_root + '/labels'
        self.imgs = []
        self.poss = []
        self.targets = []
        self.transform = transform
        ...
        for i in range(data_len):
            ...
            self.imgs.append(img)
            self.poss.append(pos)
            self.targets.append(img_root + '/{}.xml'.format(i))
...
class VOCSegFolder(Dataset):
    """
    Custom Dataset to load original img, positive generation and negative generation
    """
    def __init__(self, transform, train):
        if train:
            data_root = 'data/VOC_Seg/train'
            data_len = 5717
        else:
            data_root = 'data/VOC_Seg/test'
            data_len = 5823
        img_root = data_root + '/img'
        pos_root = data_root + '/pos'
        target_root = data_root + '/labels'
        ...
        for i in range(data_len):
            ...
            self.imgs.append(img)
            self.poss.append(pos)
            self.targets.append(img_root + '/{}.png'.format(i))
